...
if (!empty($_POST['idcategorias']) && !empty($_POST['nombre']) && !empty($_POST['descripcion']) && !empty($_POST['preciocompra']) && !empty($_POST['precioventa']) && !empty($_POST['stock'])) {
    $productoController->agregarProducto($_POST['idcategorias'], $_POST['nombre'], $_POST['descripcion'], $_POST['preciocompra'], $_POST['precioventa'], $_POST['stock']);
        ...
        <form action="agregar.php" method="POST" class="w-full">
            ...
            <div class="mb-4 flex flex-col gap-y-[10px]">
                ...
                <input type="text" id="descripcion" name="descripcion"
                    class="w-full px-3 py-2 border rounded-md focus:outline-none focus:border-blue-500"
                    placeholder="Ingrese el descripcion de la producto" 
                    require
                    >
                <input type="number" id="preciocompra" name="preciocompra"
                    class="w-full px-3 py-2 border rounded-md focus:outline-none focus:border-blue-500"
                    placeholder="Ingrese el preciocompra de la producto" 
                    require
                    >
                <input type="number" id="precioventa" name="precioventa"
                    class="w-full px-3 py-2 border rounded-md focus:outline-none focus:border-blue-500"
                    placeholder="Ingrese el precioventa de la producto" 
                    require
                    >
                <input type="number" id="stock" name="stock"
                    class="w-full px-3 py-2 border rounded-md focus:outline-none focus:border-blue-500"
                    placeholder="Ingrese el stock de la producto" 
                    require
                    >
